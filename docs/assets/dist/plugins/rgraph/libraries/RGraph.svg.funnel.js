...
(function (win, doc, undefined)
{
    RGraph.SVG.Funnel = function (conf)
    {
        ...
        this.drawFunnel = function (opt)
        {
            ...
            var centerx = prop.marginLeft + (this.graphWidth / 2);
            ...
            for (var i=0; i<(this.data.length - 1); ++i) {
                ...
                var x1 = centerx - (width / 2),
                    y1 = prop.marginTop + (height * i),
                    x2 = centerx + (width / 2),
                    y2 = prop.marginTop + (height * i);
                    x3 = centerx + (nextWidth / 2),
                    y3 = prop.marginTop + (height * (i+1)),
                    x4 = centerx - (nextWidth / 2),
                    y4 = prop.marginTop + (height * (i+1));
            ...
            for (var i=0,len=this.coords.length,sequentialIndex=0; i<len; ++i,++sequentialIndex) {
                ...
                if (i < len) {
                    ...
                    if (!RGraph.SVG.isNull(prop.tooltips) && prop.tooltips[i]) {
                ...
                if (prop.labelsPosition === 'section') {
                    ...
                    for (var i=0; i<prop.labels.length; ++i) {
                        ...
                        RGraph.SVG.text({
                            ...
                            x:          x,
                            y:          prop.marginTop + (sectionHeight / 2) + (i * sectionHeight),
                            halign:     halign,
                            valign:     'center',
                ...
                // edge Positioning
                    ...
                    for (var i=0; i<prop.labels.length; ++i) {
                        RGraph.SVG.text({
                            ...
                            text:       typeof prop.labels[i] === 'string' || prop.labels[i] === 'number' ? prop.labels[i].toString() : '',
                            x:          x,
                            y:          prop.marginTop + ((this.graphHeight / (prop.labels.length - 1) ) * i),
                            halign:     halign,
                            valign:     'center',
                            background: prop.labelsBackground || 'rgba(255,255,255,0.5)',
                            padding:    2,
        ...
        this.parseColors = function () 
        {
            ...
            if (!Object.keys(this.originalColors).length) {
                this.originalColors = {
                    ...
                    backgroundBarsColors: RGraph.SVG.arrayClone(prop.backgroundBarsColors)
                }
            }
            ...
            if (prop.backgroundBarsColors && prop.backgroundBarsColors.length) {
                for (var i=0; i<prop.backgroundBarsColors.length; ++i) {
                    prop.backgroundBarsColors[i] = RGraph.SVG.parseColorLinear({
                        object: this,
